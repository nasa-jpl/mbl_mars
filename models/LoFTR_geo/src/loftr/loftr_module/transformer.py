...
class GeoMergerTransformer(nn.Module):
    def __init__(self, config):
        super(GeoMergerTransformer, self).__init__()

        self.config = config
        self.d_model = config['d_model']
        self.encoder_layer_cross_1 = LoFTREncoderLayer(self.d_model, config['nhead'], config['attention'])
        self.encoder_layer_cross_2 = LoFTREncoderLayer(self.d_model, config['nhead'], config['attention'])
        # feed-forward network
        self.mlp = nn.Sequential(
    ...
    def forward(self, feat1, feat2, mask1=None, mask2=None):
        feat_b = self.encoder_layer_cross_1(feat1, feat2, mask1, mask2)
        feat_c = self.encoder_layer_cross_2(feat2, feat1, mask2, mask1)
        merged_feat = torch.cat([feat_b, feat_c], dim=2)
        feat_d = self.mlp(merged_feat)
        feat_d = self.norm(feat_d)
        return feat_d
