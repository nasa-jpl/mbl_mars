...
class FinePreprocess(nn.Module):
    ...
    def forward(self, feat_f0, feat_f1, feat_c0, feat_c1, data):
        ...
        if self.cat_c_feat:
            #print("CAT")
            #print(feat_c0[data['b_ids'], data['i_ids']].shape)
            #print(torch.cat([feat_c0[data['b_ids'], data['i_ids']],feat_c1[data['b_ids'], data['j_ids']]]).shape)
            feat_c_win = self.down_proj(torch.cat([feat_c0[data['b_ids'], data['i_ids']],
                                                   feat_c1[data['b_ids'], data['j_ids']]], 0))  # [2n, c]
            #print(feat_c_win.shape)
            ...
            #print("Feat_c_win:", feat_c_win.shape)

            feat_cf_win = self.merge_feat(torch.cat([
                torch.cat([feat_f0_unfold, feat_f1_unfold], 0),  # [2n, ww, cf]
                feat_c_win,  # [2n, ww, cf]
            ], -1))
            #print("Feat_cf_win:", feat_cf_win.shape)
