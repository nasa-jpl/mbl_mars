...
class LoFTRLoss(nn.Module):
    def __init__(self, config):
        ...
        if config['loftr']['contrastive'] and self.training:
            self.ce_loss = nn.CrossEntropyLoss(reduction='mean') # 'none', 'sum', 'mean'
    ...
    def _compute_fine_loss_l2(self, expec_f, expec_f_gt):
        ...
        if correct_mask.sum() == 0:
            if self.training:  # this seldomly happen when training, since we pad prediction with gt
                ...
                print("Warning: assign a false supervision to avoid ddp deadlock")
                correct_mask[0] = True
            else:
                return None
    ...
    def symmetric_ce_loss(self, logits_per_img, logits_per_depth):
        B, N, N  = logits_per_img.shape

        labels = torch.arange(N).to(logits_per_img.device).unsqueeze(0)
        labels = labels.repeat(B,1).view(B*N)
        #print(labels)
        #print(labels.shape)
        logits_per_img = logits_per_img.view(B*N, N)
        logits_per_depth = logits_per_depth.view(B*N, N)
        #print(logits_per_img.shape)
        
        loss_img = self.ce_loss(input=logits_per_img, target=labels)
        loss_depth = self.ce_loss(input=logits_per_depth, target=labels)
        loss = (loss_img + loss_depth) / 2.0
        return loss
    ...
    def forward(self, data):
        ...
        if self.config['loftr']['contrastive'] and self.training:
            loss_contrastive = self.symmetric_ce_loss(data['logits_per_img'], data['logits_per_depth'])
            loss += loss_contrastive * self.loss_config['contrastive_weight']
            loss_scalars.update({"loss_contr":  loss_contrastive.clone().detach().cpu()})
